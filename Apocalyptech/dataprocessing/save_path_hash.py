...
# Taken from ttwl-cli-saveedit/bl3-cli-saveedit
...
def main():
    ...
    parser = argparse.ArgumentParser('Compute inventory hash values')

    parser.add_argument('objpath',
            nargs=1,
            help='Object path to compute',
            )

    args = parser.parse_args()
    args.objpath = args.objpath[0]

    print(inventory_path_hash(args.objpath))
        ...
        for dirpath, dirnames, filenames in os.walk('.'):
            for filename in filenames:
                if filename.endswith('.uasset'):
                    full_path = os.path.join(dirpath[1:], filename[:-7])
