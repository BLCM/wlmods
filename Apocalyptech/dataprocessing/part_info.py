...
parser = argparse.ArgumentParser(
        description='Collect WL Part Information',
        )

parser.add_argument('-j', '--json',
        action='store_true',
        help='In addition to writing info to stdout, write a JSON file with the info',
        )

parser.add_argument('--json-output',
        type=str,
        default='part_info.json',
        help='JSON filename to write to, when writing JSON',
        )
...
part_skips = set([
    ...
    # TODO: just a BVA we don't know how to handle
    '/Game/Gear/Weapons/AssaultRifles/Vladof/_Shared/_Design/_Unique/BreadSlicer/Parts/Part_AR_VLA_Barrel_BreadSlicer',
    '/Game/Gear/Weapons/Shotguns/Jakobs/_Shared/_Design/Parts/Barrel/Barrel_04/Part_SG_JAK_Barrel_04',
    '/Game/Gear/Weapons/Shotguns/Jakobs/_Shared/_Design/_Unique/CrossBlade/Parts/Part_SG_JAK_Barrel_Crossblade',
    '/Game/PatchDLC/Indigo4/Gear/Weapons/Shotguns/Jakobs/_Shared/_Design/_Unique/MerlinsRazor/Part/Part_SG_JAK_MerlinsRazor',
    ])
...
for part_name in sorted(
        list(data.find('/Game/Gear/Weapons', 'Part_'))
        + list(data.find('/Game/PatchDLC/Indigo1/Gear/Weapons', 'Part_'))
        + list(data.find('/Game/PatchDLC/Indigo2/Gear/Weapons', 'Part_'))
        + list(data.find('/Game/PatchDLC/Indigo3/Gear/Weapons', 'Part_'))
        + list(data.find('/Game/PatchDLC/Indigo4/Gear/Weapons', 'Part_'))
        ):
    ...
    for export in part:
        if export['_jwp_object_name'].startswith('Part_') and export['export_type'].startswith('BPInvPart_'):
            ...
            for attr in report_attrs:
                for base in attr.bases:
                    json_dict[part_name].append({
                        'attr': attr.name,
                        'type': 'BaseValue',
                        'value': base,
                        })
                if attr.preadds:
                    json_dict[part_name].append({
                        'attr': attr.name,
                        'type': 'PreAdd',
                        'value': sum(attr.preadds),
                        })
                if attr.postadds:
                    json_dict[part_name].append({
                        'attr': attr.name,
                        'type': 'PostAdd',
                        'value': sum(attr.postadds),
                        })
                if attr.scales:
                    total_scale = sum(attr.scales)
                    if total_scale > 0:
                        json_dict[part_name].append({
                            'attr': attr.name,
                            'type': 'Scale',
                ...
                if attr.scalesimples:
                    json_dict[part_name].append({
                        'attr': attr.name,
                        'type': 'ScaleSimple',
                        'value': math.prod(attr.scalesimples),
                        })
...
# Otput to JSON, if we've been told to
if args.json:
    with open(args.json_output, 'w') as df:
        json.dump(json_dict, df, indent=4)
